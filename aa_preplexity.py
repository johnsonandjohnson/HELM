...
from lightening_module_aa import AntibodyLLMLightening
...
class DataAntiBody(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == "mlm":
            ...
            return torch.tensor(sequences), torch.tensor(pos_ids), torch.tensor(target), torch.tensor(seq_size)
            ...
            return torch.tensor(data[:-1]), torch.tensor(pos_ids), torch.tensor(data[1:]), torch.tensor(len(data) - 1)
...
if model == "xe":
    ...
    if "random" in model:
        from create_tree import shuffle_and_merge_codons
        tree = shuffle_and_merge_codons(all_tokens)
        weights = get_weighting(tree, "exponential", value=0.2)
...
model_config.bidirectional = True if mode == "mlm" else False
...
if model == "xe" and mode == "next_token_pred":
    path = "./downstream/gpt_clm/last.ckpt"
    model_config.max_position_embeddings = 1024
elif model == "xe" and mode == "mlm":
    path = "./downstream/gpt_mlm/last.ckpt"
    model_config.max_position_embeddings = 1024
elif model == "hxe" and mode == "mlm":
    path = "./downstream/last.ckpt"
    model_config.max_position_embeddings = 1024
elif model == "hxe" and mode == "next_token_pred":
    path = "./downstream/CLM/0.6/last.ckpt"
    model_config.max_position_embeddings = 2048
elif model == "randomhxe" and mode == "mlm":
    path = "./downstream/ranom_h_0.2/clm/last.ckpt"
    model_config.max_position_embeddings = 2048
elif model == "randomhxe" and mode == "next_token_pred":
    path = "./downstream/ranom_h_0.2/clm/last.ckpt"
    model_config.max_position_embeddings = 2048
...
data = DataAntiBody("./downstream/", "oas_codonized_v2_test_split.csv", tokenizer, CODON_TO_ID_3, 0, ["single"], 0.0, 444, mode, "csv", "codonized_all", tree, -1)
dataloader = torch.utils.data.DataLoader(data, batch_size=32, shuffle=False, num_workers=8)
t.validate(lm, dataloaders=dataloader)