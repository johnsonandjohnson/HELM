...
class HierarchicalLLLoss(torch.nn.Module):
    ...
    def forward(self, inputs, target):
        ...
        num = torch.squeeze(torch.bmm(inputs, self.onehot_num[target])).to(torch.float32)
        # sum of probabilities for denominators
        den = torch.squeeze(torch.bmm(inputs, self.onehot_den[target])).to(torch.float32)
