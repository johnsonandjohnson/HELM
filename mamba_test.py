...
from downstream.models.treeformer import TreeLMHeadModel
from models.pos_embed import PositionalEmbeddingSinCos
...
model = TreeLMHeadModel(config=config, device="cuda").cuda()
mlm_loss = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.AdamW(model.parameters(), lr=0.001)
# scheduler = LinearWarmupCosineAnnealingLR(optimizer=optimizer, warmup_epochs=5, max_epochs=-1, warmup_start_lr=1e-5, eta_min=1e-5)
...
lightning = AntibodyLLMLightening(model, "mlm", None, None, None, None, None, mlm_loss, optimizer, None, label_metrics, logits_metrics, data_type="aa")
trainer = L.Trainer()
trainer.fit(lightning, dataloader)