...
class MixerModel(nn.Module):
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int):
        num_position_embeds_diff = new_num_position_embeddings - self.max_len

        # no resizing needs to be done if the length stays the same
        if num_position_embeds_diff == 0:
            return

        old_position_embeddings_weight = self.embedding_pos.weight.clone()

        self.embedding_pos = nn.Embedding(new_num_position_embeddings, self.dim)

        with torch.no_grad():
            if num_position_embeds_diff > 0:
                self.embedding_pos.weight[:-num_position_embeds_diff] = nn.Parameter(
                    old_position_embeddings_weight
                )
            else:
                self.embedding_pos.weight = nn.Parameter(
    ...
    def forward(self, input_ids, position_ids, inference_params=None, **mixer_kwargs):
        ...
        if position_ids is None:
            position_ids = torch.arange(hidden_states.size(1), device=hidden_states.device)
